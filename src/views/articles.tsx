...
export const ArticlesGrid = ({ articles }: ArticlesGridProps) => {
  return (
    <section className="w-full max-w-6xl mx-auto py-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {articles.map((article, i) => (
        <article
          key={i}
          className="rounded-3xl bg-[var(--card)] border border-[var(--border)] p-6 hover:-translate-y-1 transition-all duration-300 hover:shadow-[0_6px_18px_rgba(0,0,0,0.6)]"
        >
          <a href={`/article/${article.id}`} className=" flex flex-col justify-between">
